...
class Net(nn.Module):
	def __init__(self,ch1,ch2,ch3):
		...
		self.block1 = nn.Sequential(nn.Conv1d(1, ch1, kernel_size=3,stride=1, padding = 1),
									...
									# nn.ReLU(),
									nn.Flatten(),
									nn.Linear(31*ch2,fc1), # (31 x 32 = 992)
									nn.ReLU(),
									nn.Linear(fc1,51),
									# nn.Linear(64,36)
									)
...
plt.ylabel("Accuracy")
plt.grid()
plt.savefig("./results/conv_1d_acc.jpg", dpi=300, bbox_inches='tight')
plt.show()
...
plt.ylabel("Loss")
plt.grid()
plt.savefig("./results/conv_1d_loss.jpg", dpi=300, bbox_inches='tight')
plt.show()
